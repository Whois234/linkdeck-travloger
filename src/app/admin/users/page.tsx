...
function ActionMenu({ user, isSelf, onEdit, onAccess, onReset, onToggle }: {
  ...
  function handleOpen() {
    if (btnRef.current) {
      ...
      // Estimate menu height: ~170px for full menu, ~130px for self (no deactivate)
      const menuH = isSelf ? 130 : 170;
      setDropUp(rect.bottom + menuH > window.innerHeight - 16);
    }
    setOpen(v => !v);
  }
    ...
    <div className="relative">
      ...
      {open && (
        <>
          ...
          <div
            className="absolute right-0 z-20 w-48 bg-white rounded-xl py-1 overflow-hidden"
            style={{
