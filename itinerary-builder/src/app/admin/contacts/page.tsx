...
function ContactPanel({
  ...
  async function assignTo(userId: string | null, userName: string) {
    setAssigning(true);
    await fetch(`/api/v1/crm/contacts/${contact.id}`, {
      method: 'PATCH', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ assigned_to_id: userId }),
    });
    setDetail(prev => prev ? { ...prev, assigned_to_id: userId, assigned_to: userId ? { id: userId, name: userName } as Contact['assigned_to'] : null } : prev);
    setAssigning(false); setShowAssign(false); onUpdated();
  }
    ...
    <div className="fixed inset-0 z-40 flex">
      ...
      <div className="w-full sm:w-[520px] sm:max-w-[90vw] bg-white flex flex-col shadow-2xl overflow-hidden" style={{ borderLeft: '1px solid #E2E8F0' }}>
              ...
              <>
                ...
                <div className="relative" ref={assignRef}>
                  <button onClick={() => setShowAssign(p => !p)}
                    className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold transition-colors hover:bg-[#EFF6FF]"
                    ...
                    {c.assigned_to?.name ?? 'Assign'}
                  </button>
                  {showAssign && (
                    <div className="absolute top-8 right-0 bg-white rounded-xl shadow-xl z-30 min-w-[180px] overflow-hidden" style={{ border: '1px solid #E2E8F0' }}>
                      <p className="px-3 py-2 text-[10px] font-bold uppercase tracking-wider" style={{ color: '#94A3B8', borderBottom: '1px solid #F1F5F9' }}>Assign to</p>
                      {users.map(u => (
                        <button key={u.id} onClick={() => assignTo(u.id, u.name)}
                          className="w-full text-left px-3 py-2 text-xs hover:bg-[#F8FAFC] flex items-center gap-2">
                          <span className="w-6 h-6 rounded-full flex items-center justify-center text-white text-[10px] font-bold flex-shrink-0"
                            style={{ backgroundColor: '#134956' }}>{u.name.charAt(0).toUpperCase()}</span>
                          <span style={{ color: '#0F172A' }}>{u.name}</span>
                          {c.assigned_to_id === u.id && <Check className="w-3 h-3 ml-auto" style={{ color: '#2563EB' }} />}
                        </button>
                      ))}
                      {c.assigned_to_id && (
                        <button onClick={() => assignTo(null, '')}
                          className="w-full text-left px-3 py-2 text-xs hover:bg-[#FEF2F2]" style={{ color: '#DC2626', borderTop: '1px solid #F1F5F9' }}>
                          Unassign
                        </button>
                      )}
                    </div>
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          {/* Quick Actions — Gallabox + WhatsApp */}
          <div className="px-6 py-3 flex gap-2" style={{ borderBottom: '1px solid #F1F5F9' }}>
            {c.phone && (
              <a
                href={`https://app.gallabox.com/conversations?search=${encodeURIComponent(c.phone.replace(/\D/g, ''))}`}
                target="_blank" rel="noopener noreferrer"
                className="flex items-center gap-1.5 px-3 py-2 rounded-lg text-xs font-semibold transition-colors hover:opacity-80 flex-1 justify-center"
