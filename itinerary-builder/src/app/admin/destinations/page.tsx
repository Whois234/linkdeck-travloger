...
export default function DestinationsPage() {
  ...
  function clearFilters() { setFilterState(''); setFilterStatus(''); setFilterDuplicates(false); setSearch(''); resetPage(); }
  ...
  const duplicateIds = useMemo(() => {
    const dupKey = (r: Dest) => `${r.name.trim().toLowerCase()}||${r.state_id}`;
  ...
  const processed = useMemo(() => {
    ...
    switch (sortKey) {
      ...
      case 'state_az':     arr.sort((a, b) => a.state.name.localeCompare(b.state.name)); break;
    ...
    return arr;
  }, [rows, search, filterState, filterStatus, filterDuplicates, duplicateIds, sortKey]);

  const totalPages = Math.ceil(processed.length / pageSize);
  const paginated = processed.slice((currentPage - 1) * pageSize, currentPage * pageSize);
  const activeFilterCount = [filterState, filterStatus, filterDuplicates ? 'dup' : ''].filter(Boolean).length;
      ...
      <div className="bg-white rounded-xl overflow-hidden" style={{ border: '1px solid #E2E8F0', ...cardShadow }}>
        ...
        <div className="px-5 py-3.5 flex flex-wrap items-center gap-3" style={{ borderBottom: '1px solid #F1F5F9' }}>
          <div className="flex items-center gap-3 flex-1 min-w-0">
            <p className="text-sm font-semibold whitespace-nowrap" style={{ color: '#64748B' }}>
              {loading ? 'Loading…' : `${processed.length} destination${processed.length !== 1 ? 's' : ''}`}
        ...
        {showFilters && (
          <div className="px-5 py-3 flex flex-wrap items-center gap-2" style={{ borderBottom: '1px solid #F1F5F9', background: '#FAFBFC' }}>
            ...
            <FilterSelect label="State" value={filterState} onChange={v => { setFilterState(v); resetPage(); }} options={stateOptions} placeholder="All States" />
            <div className="relative">
        ...
        {loading ? <div className="py-16 text-center"><div className="w-8 h-8 rounded-full border-2 border-[#134956] border-t-transparent animate-spin mx-auto" /></div>
          : processed.length === 0 ? <div className="py-16 text-center"><p className="font-semibold text-sm" style={{ color: '#0F172A' }}>No destinations found</p><p className="text-sm mt-1" style={{ color: '#64748B' }}>{activeFilterCount || search ? <button onClick={clearFilters} className="underline" style={{ color: '#134956' }}>Clear filters</button> : 'Add your first destination'}</p></div>
          : <>
                    ...
                    <tr key={r.id} className={`transition-colors ${isDup ? 'hover:bg-[#FFF1F1]' : 'hover:bg-[#F8FAFC]'}`}
                      ...
                      <td className="px-5 py-0 font-semibold" style={{ color: '#0F172A' }}>
                        <div className="flex items-center gap-1.5">
                          {isDup && <span className="text-[10px] text-red-500 flex-shrink-0" title="Duplicate destination">⚠</span>}
                          {r.name}
                        </div>
                      </td>
