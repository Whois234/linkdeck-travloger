...
interface GroupTemplate {
  ...
  state: { name: string }; state_id: string; group_template_days: Array<{ id: string }>; status: boolean;
}
const EMPTY = { group_template_name: '', state_id: '', duration_days: '1', duration_nights: '1', hero_image: '' };
...
export default function GroupTemplatesPage() {
  const [rows, setRows] = useState<GroupTemplate[]>([]);
  const [states, setStates] = useState<State[]>([]);
  const [loading, setLoading] = useState(true);
  const [showForm, setShowForm] = useState(false);
  const [editing, setEditing] = useState<GroupTemplate | null>(null);
  ...
  async function load() {
    setLoading(true);
    const [tr, sr] = await Promise.all([fetch('/api/v1/group-templates'), fetch('/api/v1/states')]);
  ...
  function openEdit(r: GroupTemplate) {
    setEditing(r);
    setForm({ group_template_name: r.group_template_name, state_id: r.state_id, duration_days: r.duration_days.toString(), duration_nights: r.duration_nights.toString(), hero_image: '' });
  ...
  async function handleSave() {
    ...
    const url = editing ? `/api/v1/group-templates/${editing.id}` : '/api/v1/group-templates';
  ...
  async function handleDelete(id: string) {
    ...
    setDeleting(id); await fetch(`/api/v1/group-templates/${id}`, { method: 'DELETE' }); setDeleting(null); load();
  }

  const filtered = rows.filter(r => !search || r.group_template_name.toLowerCase().includes(search.toLowerCase()) || r.state.name.toLowerCase().includes(search.toLowerCase()));

  return (
    <div className="max-w-[1400px]">
      <PageHeader title="Group Templates" subtitle="Fixed departure group tour templates" crumbs={[{ label: 'Admin', href: '/admin' }, { label: 'Group Templates' }]}
      ...
      />
      <Modal open={showForm} onClose={() => setShowForm(false)} title={editing ? 'Edit Group Template' : 'Add New Group Template'} subtitle="Fill in the template details">
{error && <div className="mb-4 p-3.5 rounded-lg text-sm font-medium" style={{ backgroundColor: '#FEF2F2', color: '#DC2626', border: '1px solid #FECACA' }}>{error}</div>}
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="sm:col-span-2"><label className={lbl} style={lblStyle}>Template Name <span style={{ color: '#EF4444' }}>*</span></label><input className={inp} style={inpStyle} value={form.group_template_name} onChange={e => setForm(p => ({ ...p, group_template_name: e.target.value }))} placeholder="Kerala Group Tour 6D/5N" /></div>
      ...
      <div className="bg-white rounded-xl overflow-hidden" style={{ border: '1px solid #E2E8F0', ...cardShadow }}>
        ...
        {loading ? <div className="py-16 text-center"><div className="w-8 h-8 rounded-full border-2 border-[#134956] border-t-transparent animate-spin mx-auto" /></div>
          : filtered.length === 0 ? <div className="py-16 text-center"><p className="font-semibold text-sm" style={{ color: '#0F172A' }}>No group templates found</p><p className="text-sm mt-1" style={{ color: '#64748B' }}>{search ? 'Try a different search' : 'Add your first group template'}</p></div>
          : <table className="w-full text-sm">
              <thead><tr style={{ backgroundColor: '#F8FAFC', borderBottom: '1px solid #E2E8F0' }}>{['Template Name', 'State', 'Duration', 'Days', 'Status', ''].map(h => <th key={h} className="px-5 py-3.5 text-left text-[11px] font-semibold uppercase tracking-wider whitespace-nowrap" style={{ color: '#64748B' }}>{h}</th>)}</tr></thead>
              <tbody>
                {filtered.map(r => (
                  <tr key={r.id} className="transition-colors hover:bg-[#F8FAFC]" style={{ borderBottom: '1px solid #F1F5F9', height: '56px' }}>
                    <td className="px-5 py-0 font-semibold" style={{ color: '#0F172A' }}>{r.group_template_name}</td>
                    ...
                    <td className="px-5 py-0"><span className="inline-flex items-center justify-center w-7 h-7 rounded-lg text-xs font-bold" style={{ backgroundColor: '#F1F5F9', color: '#475569' }}>{r.group_template_days.length}</span></td>
