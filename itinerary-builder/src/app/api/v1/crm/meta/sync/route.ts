...
async function graphFetch(url: string): Promise<Record<string, unknown>> {
  console.log('[META SYNC] GET', url.replace(/access_token=[^&]+/, 'access_token=***'));
  ...
  try {
    ...
    throw new Error(`Meta API returned non-JSON: ${text.slice(0, 200)}`);
  }
  // Meta wraps errors in 200 responses too — catch those
  if (data.error) {
    const e = data.error as Record<string, unknown>;
    const msg = `Meta API error ${e.code ?? ''}: ${e.message ?? JSON.stringify(e)}`;
...
async function fetchAllPages(
  ...
  while (url) {
    ...
    all.push(...items);
    console.log(`[META SYNC] ${label} page ${page} — got ${items.length} items (total so far: ${all.length})`);
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (ads.length === 0) {
      console.warn('[META SYNC] 0 ads returned — verify the ad account has active ads');
    }
    ...
    for (const ad of ads) {
      ...
      const campaignName = campaignMap[campaignId] ?? adsetInfo?.campaign_id ?? '';
