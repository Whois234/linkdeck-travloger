...
import { Plus, Search, Star, ChevronRight, Building2, LayoutGrid, List, ArrowUpDown, Trash2, SlidersHorizontal, X, AlertTriangle } from 'lucide-react';
...
export default function HotelsPage() {
  ...
  const displayed = useMemo(() => {
    const f = rows.filter(r => {
      ...
      return q && c && d && t && s && st && dup;
    });
    return sortHotels(f, sortKey);
  }, [rows, search, catFilter, destFilter, filterType, filterStars, filterStatus, filterDuplicates, duplicateIds, sortKey]);
  ...
  return (
    <div className="max-w-[1400px]">
      ...
      <div className="flex items-center gap-2 mb-3 flex-wrap">
        ...
        {/* Filters toggle */}
        {(() => {
          const activeCount = [catFilter, filterType, filterStars, filterStatus].filter(Boolean).length;
          return (
            <button onClick={() => setShowFilters(p => !p)}
              className="flex items-center gap-1.5 h-9 px-3 rounded-lg border text-sm font-medium relative"
              style={{ borderColor: activeCount ? T : '#E2E8F0', color: activeCount ? T : '#64748B', backgroundColor: activeCount ? '#EEF7F9' : 'white' }}>
              <SlidersHorizontal className="w-3.5 h-3.5" />
              Filters
              {activeCount > 0 && (
                <span className="absolute -top-1.5 -right-1.5 w-4 h-4 rounded-full text-[10px] font-bold flex items-center justify-center text-white" style={{ backgroundColor: T }}>{activeCount}</span>
              )}
            </button>
          );
        })()}
      ...
      {showFilters && (
        <div className="flex items-end gap-4 flex-wrap mb-4 px-4 py-3 rounded-xl" style={{ backgroundColor: '#F8FAFC', border: '1px solid #E2E8F0' }}>
          ...
          {(catFilter || filterType || filterStars || filterStatus) && (
            <button onClick={() => { setCatFilter(''); setFilterType(''); setFilterStars(''); setFilterStatus(''); setSelected(new Set()); }}
